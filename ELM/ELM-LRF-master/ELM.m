function [TrainingTime, TestingTime, TrainingAccuracy,TestingAccuracy, ACTUAL,PRED] = elm(TrainingData_File, TestingData_File, Elm_Type, NumberofHiddenNeurons, ActivationFunction, C)
...
%  OutputWeight=(eye(size(H,1))/C+H * H') \ H * T';      % faster method 2 //refer to 2012 IEEE TSMC-B paper
 OutputWeight=(eye(size(H,1))/(C)+H * H') \ H * T';   %implementation; one can set regularizaiton factor C properly in classification applications
