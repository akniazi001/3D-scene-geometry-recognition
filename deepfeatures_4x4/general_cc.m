...
if(mink_norm~=-1)          % minkowski norm = (1,infinity >
    ...
    white_R = power(sum(sum (double(kleur(:,:,1)).*mask_im2)),1/mink_norm);
    white_G = power(sum(sum(double(kleur(:,:,2)).*mask_im2)),1/mink_norm);
    white_B = power(sum(sum(double(kleur(:,:,3)).*mask_im2)),1/mink_norm);
